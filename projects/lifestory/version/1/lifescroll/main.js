...
var $author$project$BoundingRectangle$BoundingRectangle = F4(
...
var $author$project$DebugSettings$decoder = function () {
	var decodeQueryString = function (debug) {
		return {
			atomicUpdates: A2($elm$core$String$contains, 'atomic-updates', debug),
			grid: A2($elm$core$String$contains, 'grid', debug),
			layout: A2($elm$core$String$contains, 'layout', debug),
			log: A2($elm$core$String$contains, 'log', debug),
			reserved: A2($elm$core$String$contains, 'reserved', debug)
		};
	};
	return $elm$json$Json$Decode$oneOf(
		_List_fromArray(
			[
				A2($elm$json$Json$Decode$map, decodeQueryString, $elm$json$Json$Decode$string),
...
var $author$project$Main$decoder = $elm$json$Json$Decode$oneOf(
	_List_fromArray(
		[
			...
			$author$project$Main$PageUpdate,
			A2(
				$elm$json$Json$Decode$at,
				_List_fromArray(
					['PageUpdate']),
				$author$project$Page$decoder)),
			A2(
			$elm$json$Json$Decode$map,
			$author$project$Main$ScrollPage,
			A2(
				$elm$json$Json$Decode$at,
				_List_fromArray(
					['ScrollPage', 'viewport']),
...
var $author$project$Main$insertPattern = F3(
	function (loggingEnabled, pattern, grid) {
		var insertWithConflictLogging = F2(
...
var $author$project$BoundingRectangle$offsetBy = F2(
	function (_v0, bounds) {
		var x = _v0.a;
		var y = _v0.b;
...
var $author$project$Life$RleParser$addCells = F3(
	function (gridState, count, aliveOrDead) {
		switch (aliveOrDead.$) {
			case 'Alive':
				var newX = gridState.x + count;
				var xRange = A2($elm$core$List$range, gridState.x, newX - 1);
				var cellsToAdd = $elm$core$Set$fromList(
					A2(
						$elm$core$List$map,
						function (i) {
							return _Utils_Tuple2(i, gridState.y);
						},
						xRange));
				return $elm$parser$Parser$Loop(
					_Utils_update(
						gridState,
						{
							gridCells: A2($elm$core$Set$union, gridState.gridCells, cellsToAdd),
							x: newX
						}));
			case 'Dead':
				return $elm$parser$Parser$Loop(
					_Utils_update(
						...
						{x: gridState.x + count}));
			default:
				return A2($author$project$Life$RleParser$nextGridLine, gridState, count);
		}
	});
var $author$project$Life$RleParser$Alive = {$: 'Alive'};
var $author$project$Life$RleParser$Dead = {$: 'Dead'};
var $author$project$Life$RleParser$EmptyLine = {$: 'EmptyLine'};
...
var $author$project$Life$RleParser$cellToken = $elm$parser$Parser$oneOf(
	_List_fromArray(
		[
			A2(
			$elm$parser$Parser$map,
			function (_v0) {
				return $author$project$Life$RleParser$Dead;
			},
			$elm$parser$Parser$token('b')),
			A2(
			$elm$parser$Parser$map,
			function (_v1) {
				return $author$project$Life$RleParser$Alive;
			},
			$elm$parser$Parser$token('o')),
			A2(
			$elm$parser$Parser$map,
			function (_v2) {
				return $author$project$Life$RleParser$EmptyLine;
			},
...
var $author$project$Parser$Extra$int = $elm$parser$Parser$oneOf(
	_List_fromArray(
		[
			A2(
			...
			[
				...
				A2(
					...
					$elm$parser$Parser$succeed(
						$author$project$Life$RleParser$addCells(gridState)),
					$author$project$Parser$Extra$int),
				$author$project$Life$RleParser$cellToken),
				A2(
				$elm$parser$Parser$keeper,
				$elm$parser$Parser$succeed(
					A2($author$project$Life$RleParser$addCells, gridState, 1)),
				$author$project$Life$RleParser$cellToken),
				A2(
				$elm$parser$Parser$ignorer,
				$elm$parser$Parser$succeed(
					A2($author$project$Life$RleParser$nextGridLine, gridState, 1)),
				$elm$parser$Parser$token('$')),
				A2(
				$elm$parser$Parser$ignorer,
				$elm$parser$Parser$succeed(
...
var $author$project$Life$RleParser$rule = A2(
	$elm$parser$Parser$ignorer,
	A2(
		$elm$parser$Parser$ignorer,
		A2(
...
var $author$project$Main$insertPatterns = F2(
	function (page, life) {
		return A3(
			$elm$core$List$foldl,
			$author$project$Main$insertPattern(page.debug.log),
			life,
			$author$project$Page$patterns(page));
	});
...
var $author$project$Set$Extra$flatMap = function (f) {
	return A2(
		$elm$core$Set$foldl,
...
var $author$project$Life$ChebyshevCircle$chebyshevCircle = function (radius) {
	var shortSide = function (displacement) {
		return A2(
			$author$project$Life$ChebyshevCircle$zip,
			A2($elm$core$List$range, (-radius) + 1, radius - 1),
			A2($elm$core$List$repeat, (2 * radius) - 1, displacement));
	};
	var longSide = function (displacement) {
		return A2(
			$author$project$Life$ChebyshevCircle$zip,
			A2($elm$core$List$repeat, (2 * radius) + 1, displacement),
...
var $author$project$Life$Neighborhoods$neighbors = function (cell) {
	return A2(
		$elm$core$Set$map,
		$author$project$Vector2$add(cell),
		$author$project$Life$ChebyshevCircle$chebyshevCircle(1));
};
...
var $author$project$Life$Life$countLiveNeighbors = F2(
	function (aliveCells, cell) {
		return $elm$core$Set$size(
			A2(
				$elm$core$Set$filter,
				function (c) {
					return A2($elm$core$Set$member, c, aliveCells);
				},
				$author$project$Life$Neighborhoods$neighbors(cell)));
	});
var $author$project$Life$Life$shouldBeBorn = F2(
	function (aliveCells, deadCell) {
		var aliveNeighbors = A2($author$project$Life$Life$countLiveNeighbors, aliveCells, deadCell);
		return aliveNeighbors === 3;
	});
var $author$project$Life$Life$shouldSurvive = F2(
	function (aliveCells, aliveCell) {
		var aliveNeighbors = A2($author$project$Life$Life$countLiveNeighbors, aliveCells, aliveCell);
		return (aliveNeighbors === 2) || (aliveNeighbors === 3);
	});
var $author$project$Life$Life$next = function (grid) {
	var survivors = A2(
		$elm$core$Set$filter,
		$author$project$Life$Life$shouldSurvive(grid),
		grid);
	var adjacentDeadCells = A2(
		$elm$core$Set$diff,
		A2($author$project$Set$Extra$flatMap, $author$project$Life$Neighborhoods$neighbors, grid),
		grid);
	var newborns = A2(
		$elm$core$Set$filter,
		$author$project$Life$Life$shouldBeBorn(grid),
		adjacentDeadCells);
...
var $author$project$ScrollState$update = F2(
	function (position, _v0) {
		var furthest = _v0.furthest;
		return {
			furthest: A2($elm$core$Basics$max, position, furthest),
			mostRecent: position
		};
	});
			...
			case 'PageUpdate':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							life: A2($author$project$Main$insertPatterns, page, model.life),
							page: A3($author$project$DebugSettings$withLogging, page.debug.log, 'page', page)
						}),
...
var $author$project$Main$canvasDimensions = function (rect) {
	return _Utils_Tuple2(
		$elm$core$Basics$ceiling(
			$author$project$BoundingRectangle$width(rect)),
		$elm$core$Basics$ceiling(
			$author$project$BoundingRectangle$height(rect)));
};
...
var $author$project$Canvas$Renderable$empty = A2($joakin$elm_canvas$Canvas$shapes, _List_Nil, _List_Nil);
...
var $author$project$Life$Life$render = F3(
	function (viewport, cellSize, cells) {
		var square = F2(
			function (point, size) {
				return A3($joakin$elm_canvas$Canvas$rect, point, size, size);
			});
		var renderCell = function (position) {
			return A2(
				square,
				_Utils_Tuple2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							A2($joakin$elm_canvas$Canvas$Settings$Advanced$translate, -viewport.left, -viewport.top)
						]))
				]),
			A2(
				$elm$core$List$map,
...
var $author$project$Life$Debug$renderGrid = F3(
	function (viewport, cellSize, page) {
		var verticalLine = function (x) {
			return A2(
				$joakin$elm_canvas$Canvas$path,
				_Utils_Tuple2((cellSize * x) - $author$project$Life$Debug$debugStrokeHalfWidth, cellSize * page.top),
				_List_fromArray(
					[
						$joakin$elm_canvas$Canvas$lineTo(
						_Utils_Tuple2((cellSize * x) - $author$project$Life$Debug$debugStrokeHalfWidth, cellSize * page.bottom))
					]));
		};
		var verticalLines = A2(
			$elm$core$List$map,
			A2($elm$core$Basics$composeL, verticalLine, $elm$core$Basics$toFloat),
			A2(
				$elm$core$List$range,
				0,
				$elm$core$Basics$ceiling(
					$author$project$BoundingRectangle$width(page) / cellSize)));
		var horizontalLine = function (y) {
			return A2(
				$joakin$elm_canvas$Canvas$path,
				_Utils_Tuple2(cellSize * page.left, (cellSize * y) - $author$project$Life$Debug$debugStrokeHalfWidth),
				_List_fromArray(
					[
						$joakin$elm_canvas$Canvas$lineTo(
						_Utils_Tuple2(cellSize * page.right, (cellSize * y) - $author$project$Life$Debug$debugStrokeHalfWidth))
					]));
		};
		var horizontalLines = A2(
			$elm$core$List$map,
			A2($elm$core$Basics$composeL, horizontalLine, $elm$core$Basics$toFloat),
			A2(
				$elm$core$List$range,
				0,
				$elm$core$Basics$ceiling(
					$author$project$BoundingRectangle$height(page) / cellSize)));
		return A2(
			$joakin$elm_canvas$Canvas$shapes,
			_List_fromArray(
				[
					$joakin$elm_canvas$Canvas$Settings$stroke($avh4$elm_color$Color$darkGray),
					$joakin$elm_canvas$Canvas$Settings$Line$lineWidth($author$project$Life$Debug$debugStrokeHalfWidth * 2),
					$joakin$elm_canvas$Canvas$Settings$Advanced$transform(
					_List_fromArray(
						[
							A2($joakin$elm_canvas$Canvas$Settings$Advanced$translate, -viewport.left, -viewport.top)
			...
			A2($elm$core$List$append, verticalLines, horizontalLines));
	});
var $author$project$Life$Debug$renderGridBounds = F4(
	function (viewport, cellSize, color, gridBoundingRectangles) {
		var gridBoundsShape = function (bounds) {
			return A3(
				$joakin$elm_canvas$Canvas$rect,
				_Utils_Tuple2((bounds.left * cellSize) - $author$project$Life$Debug$debugStrokeHalfWidth, (bounds.top * cellSize) - $author$project$Life$Debug$debugStrokeHalfWidth),
				($author$project$BoundingRectangle$width(bounds) * cellSize) + $author$project$Life$Debug$debugStrokeHalfWidth,
				($author$project$BoundingRectangle$height(bounds) * cellSize) + $author$project$Life$Debug$debugStrokeHalfWidth);
		};
		return A2(
			$joakin$elm_canvas$Canvas$shapes,
			_List_fromArray(
			...
			A2($elm$core$List$map, gridBoundsShape, gridBoundingRectangles));
	});
...
var $author$project$Life$Debug$renderLayoutRegions = F2(
	function (viewport, anchors) {
		var renderLayoutRegion = function (anchor) {
			var anchorHalfWidth = anchor.bounds.left + ($author$project$BoundingRectangle$width(anchor.bounds) / 2);
			return _List_fromArray(
				[
					A3(
					$joakin$elm_canvas$Canvas$rect,
					_Utils_Tuple2(anchor.bounds.left, anchor.bounds.top),
					$author$project$BoundingRectangle$width(anchor.bounds),
					$author$project$BoundingRectangle$height(anchor.bounds)),
					A2(
					$joakin$elm_canvas$Canvas$path,
					_Utils_Tuple2(anchorHalfWidth, anchor.bounds.top),
					_List_fromArray(
						[
							$joakin$elm_canvas$Canvas$lineTo(
							_Utils_Tuple2(anchorHalfWidth, anchor.bounds.bottom))
						]))
				]);
		};
		return A2(
			...
			A2($elm$core$List$concatMap, renderLayoutRegion, anchors));
	});
...
var $author$project$Main$view = function (_v0) {
	...
	return A3(
		...
		$author$project$Main$canvasDimensions(viewport),
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(page.body.left, page.body.top),
				$author$project$BoundingRectangle$width(viewport),
				$author$project$BoundingRectangle$height(viewport)),
				A3($author$project$Life$Life$render, viewport, page.cellSizeInPixels, life.cells),
				page.debug.grid ? A3($author$project$Life$Debug$renderGrid, viewport, page.cellSizeInPixels, page.body) : $author$project$Canvas$Renderable$empty,
				page.debug.atomicUpdates ? A4(
				$author$project$Life$Debug$renderGridBounds,
				viewport,
				page.cellSizeInPixels,
				$avh4$elm_color$Color$red,
				A2(
					$elm$core$List$map,
					function (_v1) {
						var bounds = _v1.bounds;
						return bounds;
					},
					life.atomicUpdateRegions)) : $author$project$Canvas$Renderable$empty,
				page.debug.layout ? A2($author$project$Life$Debug$renderLayoutRegions, viewport, page.anchors) : $author$project$Canvas$Renderable$empty
			]));
};
